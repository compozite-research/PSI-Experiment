...
std::vector<oc::block> loadSetFromCsv(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        std::string item;
        std::getline(ss, item, ',');  // Assuming single column CSV
        set.push_back(oc::block(0, std::stoull(item)));  // Convert to oc::block
    }
...
void messagePassingExampleBoth(oc::CLP& cmd)
{
    ...
    senderSet = loadSetFromCsv("../data/sender_set.csv");
    ...
    recverSet.resize(nr);
    recverSet = loadSetFromCsv("../data/receiver_set.csv");
